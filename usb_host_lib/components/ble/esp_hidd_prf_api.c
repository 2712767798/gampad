...
uint8_t mouse_buffer[HID_MOUSE_IN_RPT_LEN];
...
void esp_hidd_send_mouse_value(uint16_t conn_id,bool t,void* data) //uint8_t mouse_button, int8_t mickeys_x, int8_t mickeys_y)
{
    memcpy(mouse_buffer,(uint8_t*)data,HID_MOUSE_IN_RPT_LEN);
    //ESP_LOG_BUFFER_HEX("mouse_buffer",mouse_buffer,HID_MOUSE_IN_RPT_LEN);
    // mouse_buffer[1]=0X00;
    // mouse_buffer[2]=0X02;
    // mouse_buffer[3]=0;
    // mouse_buffer[4]=0;
    ...
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_RPT_ID_MOUSE_IN, HID_REPORT_TYPE_INPUT, HID_MOUSE_IN_RPT_LEN, mouse_buffer);
    return;
}
